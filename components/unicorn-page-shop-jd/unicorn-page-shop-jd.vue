	...
	<view>
		...
		<view style="margin-top: 0rpx;">
			<view class="unicorn-header1">
				<scroll-view scroll-x class="text-white nav bg-blue" scroll-with-animation :scroll-left="scrollLeft"
					style="background: transparent !important;">
					<view class="cu-item" :class="index==TabCur?'unicorn-font-size cur':''"
						v-for="(item,index) in tabList" :key="index" @tap="tabSelect" :data-id="index">
						{{item.name}}
					</view>
				</scroll-view>
				<swiper class="screen-swiper square-dot" :indicator-dots="true" :circular="true" v-if="TabCur==0"
					:autoplay="true" interval="5000" duration="500"
					style="height: 120px;min-height: 100px;margin-top: 5px;">
			...
			<view v-if="TabCur==0">
				<view class="unicorn-header2" style="margin-top: -1px;">
					<swiper class="screen-swiper square-dot" :indicator-dots="true" :circular="false" :autoplay="false"
						interval="5000" duration="500"
						style="min-height: 200rpx;background: transparent !important;height: 420rpx;">
						<swiper-item v-for="(item,index) in menuList" :key="index" class="unicorn-border-radius-swiper"
							style="background: transparent !important;">
							<view class="cu-list grid col-5 no-border" style="background: transparent !important;">
								<view class="cu-item" v-for="(itemsub,indexsub) in item" :key="indexsub"
									style="display: flex;align-items: center;justify-content: center;background: transparent !important;">
									<image mode="aspectFill" :src="itemsub.url"
										style="width: 100rpx;height: 100rpx;margin-bottom: 10rpx;border-radius: 50%;background: transparent !important;">
									</image>
				...
				<image mode="widthFix" src="/static/unicorn-page-shop-jd/title3.jpg"
					style="background: transparent !important;width: 100%;margin-top: -1px;"></image>
				...
				<view class="unicorn-body">
					...
					<view class="cu-bar bg-white unicorn-br-1 unicorn-br-2">
						...
						<view class="action">发现更多频道<text class="cuIcon-roundrightfill lg text-red"
								style="color: #F34C77;"></text></view>
					</view>
					<scroll-view class="floor-list  unicorn-br-3 unicorn-br-4" scroll-x
						style="background-color: #FFFFFF;margin-bottom: 24rpx;">
						<view class="scoll-wrapper">
							<view v-for="(item, index) in pindaoList" :key="index" class="floor-item"
					...
					<view class="guess-section unicorn-br-3 unicorn-br-4">
						<view v-for="(item, index) in goodsList" :key="index" class="guess-item"
							...
							<view class="bg-white" style="padding: 3px 0;padding-left: 5px;">
								<view class='cu-tag round bg-red sm' v-if="item.tag" style="background-color: #F65E2C;">
									{{item.tag}}</view>
								<text class="title" style="line-height: 50rpx;">{{item.title}}</text>
							</view>
							<text class="text-price price"
								style="color: #e54d42;padding-left: 5px;">{{item.price}}</text>
						</view>
					</view>
				</view>
			...
			<view v-if="TabCur==1" style="padding: 0 10px;margin-top: 8px;">
				...
				<view class="cu-list grid col-4 no-border unicorn-br-3 unicorn-br-4" style="padding-top: 0px;">
					<view class="cu-item" v-for="(item,index) in ppjxList" :key="index"
				...
				<scroll-view class="floor-list  unicorn-br-3 unicorn-br-4" scroll-x style="background-color: #FFFFFF;">
					<view class="scoll-wrapper">
						<view v-for="(item, index) in xsqgList" :key="index" class="floor-item"
							style="width: 185rpx;padding: 0 10px;">
							<image :src="item.url" mode="aspectFill" style="width: 185rpx;"></image>
							<text class="title clamp text-price price"
								style="text-align: center;color: #e54d42;font-size: 14px;">{{item.price}}</text>
							<text class="title clamp text-price"
								style="text-align: center;text-decoration: line-through;">{{item.oldPrice}}</text>
							<view class="pro-box">
				...
				<view class="cu-list grid col-5 no-border unicorn-br-3 unicorn-br-4" style="padding-top: 0px;">
					<view class="cu-item" v-for="(item,index) in sjpjList" :key="index"
						style="display: flex;align-items: center;justify-content: center;">
						<image class="unicorn-image" mode="aspectFill" :src="item.url" style="width: 100rpx;"></image>
						<text>{{item.name}}</text>
					</view>
				...
				<view class="cu-list grid col-3 no-border unicorn-br-3 unicorn-br-4" style="padding-top: 0px;">
					<view class="cu-item" v-for="(item,index) in sjb1List" :key="index"
						style="display: flex;align-items: center;justify-content: center;">
						<image class="unicorn-image" mode="aspectFill" :src="item.url"
							style="width: 200rpx;height: 200rpx;"></image>
						<text class="text-price" style="color: #e54d42;">{{item.price}}</text>
					</view>
				</view>

				<view class="cu-list grid col-5 no-border unicorn-br-1 unicorn-br-2 unicorn-br-3 unicorn-br-4"
					style="margin-top: 8px;">
					<view class="cu-item" v-for="(item,index) in sjb2List" :key="index"
						style="display: flex;align-items: center;justify-content: center;">
						<image class="unicorn-image" mode="aspectFill" :src="item.url"
				...
				<view class="guess-section unicorn-br-3 unicorn-br-4">
					<view v-for="(item, index) in jdhxList" :key="index" class="guess-item"
						@click="navToDetailPage(item)">
						<view class="image-wrapper">
							<image :src="item.image" mode="aspectFill"></image>
						</view>
	...
	export default {
		...
		methods: {
			...
			navToDetailPage(item){
				let g = JSON.parse(JSON.stringify(item))
				uni.navigateTo({
					url: '/pages/Detail/Detail?id='+g.id
				});
			},
			goSearchClick(item){
				this.goodName = item;
			...
			search(){
				this.$request({
					...
					const good_data = res.data;
					this.goodsList = good_data.map(item=>{
						item.image = item.goodPic;
						item.title = item.goodName;
						item.tag = "自营";
						return item;
					})
