<template>
	<view>
		...
		<view v-for="(item, index) in videoList" :key="index" class="videoIndexFlag">
			<video :src="item.src" :id="'myVideo' + index"  @error="videoErrorCallback(this, item)" :muted="nowVideoMutedStatus" :show-center-play-btn="false"
...
<script>
	import unicornJson from './unicorn-page-video-slide.json';
	export default {
		data() {
			return {
